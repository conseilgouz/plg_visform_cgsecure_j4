<?php
/**
* Visforms Check IP Plugin  - Joomla 4.x/5.x Plugin
* Package			: Visform Checkip Plugin
...
class plgvisformscgsecureInstallerScript
{
	...
	private $name                    = 'Plugin VisForm CheckIP';
	private $exttype                 = 'plugin';
	private $extname                 = 'cgsecure';
	private $previous_version        = '';
	private $dir           = null;
	private $lang;
	private $installerName = 'plgvisformcgsecureinstaller';
	...
	private function postinstall_cleanup() {
		...
		{
			$f = JPATH_SITE . '/plugins/plg_visforms_'.$this->extname.'/' . $folder;
		...
		$langFiles = [
			sprintf("%s/language/en-GB/en-GB.plg_visforms_%s.ini", JPATH_ADMINISTRATOR, $this->extname),
			sprintf("%s/language/en-GB/en-GB.plg_visforms_%s.sys.ini", JPATH_ADMINISTRATOR, $this->extname),
			sprintf("%s/language/fr-FR/fr-FR.plg_visforms_%s.ini", JPATH_ADMINISTRATOR, $this->extname),
			sprintf("%s/language/fr-FR/fr-FR.plg_visforms_%s.sys.ini", JPATH_ADMINISTRATOR, $this->extname),
		];
        ...
        try {
            ...
            Log::add('unable to enable plugin visforms cgsecure', Log::ERROR, 'jerror');
        }
		...
		$query = $db->getQuery(true)
			->delete('#__update_sites')
			->where($db->quoteName('location') . ' like "%conseilgouz.com/updates/plg_visforms_cgsecure%"');
		$db->setQuery($query);
		$db->execute();
